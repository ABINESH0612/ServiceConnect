...
export default function SupportTicketDetail() {
  ...
  const { data: ticket, isLoading, error } = useQuery({
    ...
    queryFn: () => agentTicketApi.getAssignedById(ticketId),
    select: (r) => r.data,
  ...
  const { data: messages } = useQuery({
    ...
    queryFn: () => agentTicketApi.getMessages(ticketId, { size: 100 }),
    select: (r) => r.data,
    enabled: !isNaN(ticketId),
  ...
  const sendMutation = useMutation({
    mutationFn: () => agentTicketApi.sendMessage(ticketId, { content: message }),
    onSuccess: () => { setMessage(''); qc.invalidateQueries({ queryKey: ['support', 'ticket', ticketId, 'messages'] }) },
    onError: () => toast.error('Failed to send'),
  })

  const updateMutation = useMutation({
    mutationFn: (status: TicketStatus) => agentTicketApi.updateAssigned(ticketId, { status }),
    onSuccess: (_, s) => { toast.success(`Status: ${s}`); setNewStatus(''); qc.invalidateQueries({ queryKey: ['support', 'ticket', ticketId] }) },
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <Link to="/support/tickets" className="inline-flex items-center gap-1 text-sm text-[#64748B] hover:text-[#2563EB]"><ArrowLeft className="w-4 h-4" /> Back</Link>
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div><h1 className="text-xl font-bold">{ticket.subject}</h1><p className="text-xs text-[#94A3B8]">#{ticket.ticketNumber}</p></div>
        <div className="flex gap-2"><PriorityBadge priority={ticket.priority} /><TicketStatusBadge status={ticket.status} /></div>
      </div>
      <div className="sc-card p-4"><p className="text-sm">{ticket.description}</p></div>
      ...
      <div className="sc-card p-4 flex items-center gap-3">
        ...
        <button onClick={() => newStatus && updateMutation.mutate(newStatus as TicketStatus)} disabled={!newStatus} className="sc-btn-primary text-sm">Update</button>
      </div>
        ...
        <div className="p-4 space-y-3 max-h-[400px] overflow-y-auto">
          {!msgList.length ? <p className="text-sm text-[#64748B] text-center py-4">No messages.</p> : msgList.map((m) => (
            <div key={m.id} className={`flex ${m.senderId === user?.id ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] px-3 py-2 rounded-[12px] text-sm ${m.senderId === user?.id ? 'bg-[#2563EB] text-white' : 'bg-[#F1F5F9] text-[#0F172A]'}`}>
                <p>{m.content || m.message}</p><p className={`text-xs mt-1 ${m.senderId === user?.id ? 'text-white/60' : 'text-[#94A3B8]'}`}>{formatDateTime(m.createdAt)}</p>
              </div>
            </div>
          ))}
          <div ref={messagesEnd} />
